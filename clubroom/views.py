...
class Reservation(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        for existing_reservation in B303Reservation.objects.filter(date=date):
            if (
                reservation.start_time < existing_reservation.end_time
                and reservation.end_time > existing_reservation.start_time
                and existing_reservation.pk != reservation.pk
            ):
                messages.error(
                    self.request, "「" + existing_reservation.title + "」と予約の時間が重複しています。"
                )
                return super().form_invalid(form)
        form.save()
