...
def qrcode_generator(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            qr_data.add_data(content)
            qr_data.make(fit=True)
            qrcode_img = qr_data.make_image(fill_color=qrcolor, back_color=bgcolor)
            buffer = BytesIO()
    ...
    def form_valid(self, form):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            for page_num, page in enumerate(pages, 1):
                ...
                canvas.drawString(
                    page_width / 2 - text_width / 2, margin_bottom, footer_text
                )
                canvas.restoreState()
                canvas.showPage()
            canvas.save()
            ...
            response = StreamingHttpResponse(
                FileWrapper(open(path, "rb"), 8192), content_type="application/pdf"
            )
